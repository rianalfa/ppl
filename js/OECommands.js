...
function Commandclass()
{
    ...
    if (currConfig()=="smartphone")
      ...
      // this.htmlheader+= '<style>table {width:80%;} img {width:10%; } </style>\n' ;
       this.htmlheader+= '<style>{img {width:10%; } </style>\n' ;
...
{
 // alert("at 452 in OECmds, device="+device+", ScreenConfig=" +ScreenConfig+" screen.width="+screen.width);
if ((currConfig()=="smartphone"))
  {
...
// in the precmds array.  Note: This actually creates it as the LAST command, not the first. ???BUG
...
{
if (typeof window.t == 'function') {content=t(content)};  //Translate
//Put title in a cell spanning all the rows
this.precmds[this.precmds.length]=
    'myTable.setHeaderStyle("#cccccc","center","middle",6,"arial","16px")'
