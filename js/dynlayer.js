	...
	if (is.ns4)
	  {
		...
		this.y = this.css.top
		this.w = this.css.clip.width +10;
		this.h = this.css.clip.height+10;
		}
	}
	//bc:else if (is.ie) {
...
function DynLayerClipInit(clipTop,clipRight,clipBottom,clipLeft) {
  return;  //eliminate clipping
  clipRight+=10;clipBottom+=10;  //Add some pixels to allow box shadow
	//bc:if (is.ie) {
	if (is.ie||is.ns5) {
		if (arguments.length==4) this.clipTo(clipTop,clipRight,clipBottom,clipLeft)
		else if (is.ie4) this.clipTo(0,this.css.pixelWidth,this.css.pixelHeight,0)
    //bc:
		else if (is.ns5) this.clipTo(0,this.elm.offsetWidth+10,this.elm.offsetHeight+10,0)
	}
}
function DynLayerClipTo(t,r,b,l) {
	if (t==null) t = this.clipValues('t')
	if (r==null) r = this.clipValues('r')+10;
	if (b==null) b = this.clipValues('b')+10;
